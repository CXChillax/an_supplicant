...
class SupplicantFrame(wx.Frame):
    ...
    def __init__(self, title, pos, size):
        ...
        panel = wx.Panel(self) 
        self.connect = wx.Button(panel,label=u"登录",pos=(38, 196),size=(80, 25)) 
        self.disconnect = wx.Button(panel,label=u"下线",pos=(122,196),size=(80,25))
        ...
        if self.savepass == "1":
            self.sp.SetValue(1)
            self.pwd = wx.TextCtrl(panel, -1, self.p , pos=(90,90),style=wx.TE_PASSWORD |wx.TE_PROCESS_ENTER)
        else:
            self.sp.SetValue(0)
            self.p=''
            self.pwd = wx.TextCtrl(panel, -1, self.p , pos=(90,90),style=wx.TE_PASSWORD |wx.TE_PROCESS_ENTER)
    ...
    def EvtCheckBox(self,event):  
        if self.sp.IsChecked():
            confrw.confwritesp('1')
        else:
            confrw.confwritesp('0')
    ...
    def updateconf(self):
        ...
        self.u = self.a[0]
        self.p = self.a[1]
        self.p = encrypt.decryptpass(self.p)
        self.host = self.a[2]    
        self.ver=self.a[3]
        self.ser=self.a[4]
    ...
    def _EnableOrDisableOkBtn(self):
        ...
        pwdStr = encrypt.encryptpass(pwdStr)
    ...
    def OnSet(self,event):
        self.updateconf()
        windows=TextEntryDialog(None,u'偏好设置',u'服务器IP',u'客户端版本号',u'设置服务类型(int or internet)')
